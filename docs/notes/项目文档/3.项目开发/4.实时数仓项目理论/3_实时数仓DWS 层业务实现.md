...
footer: Spring基础
copyright: bugcode
createTime: 2026/01/17 20:14:52
permalink:  /project-docs/project/RealtimeWarehouse/实时数仓DWS层业务实现/
---
...
3. 对合并的流进行聚合，**聚合的时间窗口**决定了数据的时效性，在离线数仓中，我们是按照天进行轻度的汇总，但是在实时数据仓库中，我们一般按照小时进行汇总。
4. 把聚合结果写在数据库中(ck数据库).
