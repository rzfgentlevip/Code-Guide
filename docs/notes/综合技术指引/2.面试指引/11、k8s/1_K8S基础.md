...
permalink: /compre-guide/interview/k8s/k8s基础1/
---
...
节点亲和性（Node Affinity）： NodeAffinity亲和性调度:亲和性调度机制极大的扩展了Pod的调度能力,目前有两种节点亲和力表达:
...
### 18、你知道的几种CNI网络插件,并详述其工作原理。K8s常用的CNI网络插件 (calico && flannel),简述一下它们的工作原理和区别。
...
### 20、你知道的K8s中几种Controller控制器,并详述其工作原理
...
2. StatefullSet:适合有状态的服务部署,即部署有状态应用,解决Pod的独立生命周期,保持Pod启动顺序和唯一性。
...
一次部署,所有的node节点都会部署,例如一些典型的应用场景:运行集群存储 daemon,例如在每个Node上运行 glusterd、ceph
...
- 由于service的实现方式:user_space 、iptebles、ipvs、这三种方式只支持在4层协议通信,不支持7层协议,因此NodePort不能代理https服务。
...
### 25、k8s提供了大量的REST接口,其中有一个是Kubernetes Proxy API接口,简述一下这个Proxy接口的作用,以及怎么使用。
...
### 32、pod的存活探针有哪几种？(必须记住3种探测方式,重点,经常问)
...
### 34、pod的就绪探针有哪几种？(必须记住3种探测方式,重点,经常问)
...
### 41、pod一直处于pending状态一般有哪些情况,怎么排查？(重点,持续更新)
...
### 44、pod的定义中有个command和args参数,这两个参数不会和docker镜像的entrypointc冲突吗？
...
### 49、一个应用pod是如何发现service的,或者说,pod里面的容器用于是如何连接service的？
...
### 50、如何创建一个service代理外部的服务,或者换句话来说,在k8s集群内的应用如何访问外部的服务,如数据库服务,缓存服务等?
...
这是最标准的K8s方式，手动创建Service和Endpoints对象。

```yaml
# 1. 创建Service（没有selector）
apiVersion: v1
kind: Service
metadata:
  name: external-mysql
...
subsets:
  - addresses:
    - ip: 10.0.0.123  # 外部服务的IP地址
    ports:
    - port: 3306
      protocol: TCP
```

### 51、service、endpoint、kube-proxys三种的关系是什么？
...
### 52、无头service和普通的service有什么区别,无头service使用场景是什么？
...
### 55、deployment的滚动更新策略有两个特别主要的参数,解释一下它们是什么意思？
...
### 64、怎么使一个node脱离集群调度,比如要停机维护单又不能影响业务应用
...
### 66、k8s生产中遇到什么特别影响深刻的问题吗,问题排查解决思路是怎么样的？(重点)
...
### 67、etcd集群节点可以设置为偶数个吗,为什么要设置为奇数个呢？
...
### 70、etcd集群节点之间是怎么同步数据的？

(待补充)

### 71、pod的网络是怎么实现的,比如在node节点上ping一个pod的ip,这个网络流程是什么验证的？



### 72、用户访问我们的k8s集群里面的应用网站,出现500报错,你是如何排查这种问题的？
