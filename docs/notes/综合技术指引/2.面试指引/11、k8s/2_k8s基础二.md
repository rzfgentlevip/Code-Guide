...
permalink: /compre-guide/interview/k8s/K8S基础二/
---

### 1、Kubernetes中Pod是什么,它与容器有什么区别？
...
### 3、Kubernetes中的Ingress是什么,它如何工作？
...
### 5、Kubernetes中的ConfigMap和Secret,它们有何区别？
...
### 6、Kubernetes的命名空间(Namespace)及其用途
...
### 7、Kubernetes的DaemonSet是什么,它的应用场景有哪些？
...
### 8、Kubernetes中的StatefulSet及其与Deployment的区别
...
### 9、Kubernetes中的Service Mesh是什么？它的作用是什么？
...
### 10、Kubernetes的Persistent Volume (PV) 和Persistent Volume Claim (PVC) 及它们之间的关系
...
### 11、Kubernetes的Affinity和Anti-Affinity规则是什么？它们的应用场景有哪些？
...
### 12、Kubernetes中的Horizontal Pod Autoscaler (HPA) 和 Vertical Pod Autoscaler (VPA) 有何区别？
...
### 13、Kubernetes中的Taints和Tolerations是什么？它们是如何工作的？
...
### 14、Kubernetes中的Jobs和CronJobs及它们的用途
...
### 15、Kubernetes的网络策略是什么？它们如何用于控制Pod间的通信？
...
### 16、Kubernetes中的Resource Quotas机制
...
### 17、Kubernetes中的Labels和Annotations有什么区别？
...
### 18、Kubernetes中的Init Containers及其用途
...
### 19、Kubernetes中的Headless Service是什么,与普通Service有什么不同？
...
### 20、Kubernetes中的livenessProbe和readinessProbe的作用
...
### 21、Kubernetes的Node Affinity与Pod Affinity/Anti-Affinity有何不同？
...
### 22、Kubernetes的Deployment滚动更新策略
...
### 23、Kubernetes中ConfigMap和Secret的使用方式有什么区别？
...
### 24、Kubernetes中的Volume和PersistentVolume的区别是什么？
...
### 26、Kubernetes中的Resource Limits和Requests
...
### 27、Kubernetes中的StatefulSet和Deployment的更新策略有何区别？
...
### 28、Kubernetes中的Network Policy的主要用途和工作原理
...
### 29、Kubernetes中StatefulSet和Deployment的滚动更新有何不同？
...
### 32、Kubernetes中的ServiceAccount及其用途
...
### 33、Kubernetes中的Custom Resource Definition(CRD)
...
### 34、Kubernetes中的PodPreset是什么？它是如何工作的？
...
### 36、Kubernetes中的PodDisruptionBudget(PDB)是什么？它的作用是什么？
...
### 37、Kubernetes中的Horizontal Pod Autoscaling(HPA)的度量指标
...
### 38、Kubernetes中的Affinity和Anti-Affinity规则
...
### 40、Kubernetes中的Quota(资源配额)和LimitRange(限制范围)有什么区别？

Quota(资源配额)和LimitRange(限制范围)是Kubernetes中两种资源控制机制,它们的主要区别如下:
...
### 41、Kubernetes中的DaemonSet有哪些常见的使用场景？
...
### 42、Kubernetes的ConfigMap和Secret如何在Pod中使用？
...
### 43、Kubernetes中的ReplicaSet与ReplicationController的区别
...
### 44、Kubernetes中,如何使用Init Containers初始化Pod？
...
### 47、Kubernetes中的Service Mesh是什么？它提供了哪些功能？
...
### 48、Kubernetes中的Pod优先级和抢占机制是如何工作的？
...
### 49、Kubernetes中的Node Selector与Node Affinity的区别
...
### 50、Kubernetes中,如何配置Pod以使用特定的网络策略？
...
### 51、Kubernetes中的Rolling Update是什么？它是如何进行的？
...
### 52、Kubernetes中,如何使用Persistent Volumes (PV) 和Persistent Volume Claims (PVC)？
...
### 55、**Kubernetes 的底层原理中,Pod 唯一 IP 设计是如何保证网络稳定性的？**
...
### 56、**在 Kubernetes 中,如何保证 Pod 重启后其 IP 地址不变？**
...
| 插件名称           | 核心技术与原理                       | 关键特性                                             | 网络策略支持              | 适用场景                                                     |
| :----------------- | :----------------------------------- | :--------------------------------------------------- | :------------------------ | :----------------------------------------------------------- |
| **Flannel**        | 覆盖网络 (Overlay)，默认VXLAN模式    | **简单、轻量、易部署**                               | **不支持**                | 小规模集群、测试环境、对网络功能要求不复杂的场景             |
| **Calico**         | 纯路由 (BGP) 或轻量覆盖 (IPIP/VXLAN) | **高性能、丰富的网络策略 (Network Policy)**          | **支持** (强大且灵活)     | 中大型生产集群、对安全和策略要求高的场景                     |
| **Cilium**         | 基于 **eBPF** 技术的数据平面         | **极高性能、强大的可观测性、API级网络策略 (L3-L7)**  | **支持** (强大，可到L7层) | 大规模集群、对性能、安全性和可观测性有极致要求的环境         |
| **Weave Net**      | 覆盖网络 (Overlay)，支持加密         | **支持网络加密、内置DNS、跨主机通信稳定**            | **支持** (基本)           | 中小规模集群，需要快速部署或对网络加密有要求的场景           |
| **Antrea**         | 基于 **Open vSwitch (OVS)**          | 高性能，与Kubernetes原生集成良好                     | **支持**                  | 利用OVS成熟能力，需要细粒度网络策略控制的场景                |
| **Canal**          | **Flannel + Calico** 的组合          | 兼具Flannel的部署简单和Calico的网络策略              | **支持** (通过Calico)     | 希望获得网络策略能力，同时又偏爱Flannel简洁性的场景          |
| **Kube-OVN**       | 基于 **OVN/OVS**                     | **功能丰富**：子网、静态IP、ACL、QoS等               | **支持**                  | 需要传统网络高级功能（如虚拟网络隔离、QoS）的场景            |
| **OVN-Kubernetes** | 基于 **OVN (Open Virtual Network)**  | 提供覆盖网络实现，内置OVS负载均衡器                  | **支持**                  | 与Open vSwitch生态紧密集成，需要其网络虚拟化能力的场景       |
| **Multus**         | **多网卡插件 (Meta-plugin)**         | 允许一个Pod附加多个网络接口，支持其他CNI作为“从”接口 | **取决于其调用的CNI**     | 需要SR-IOV、DPDK等高性能网络，或Pod需要接入多个专用网络的场景 |
| **Terway**         | 阿里云**VPC原生**网络插件            | 高性能，直接在VPC内分配IP，支持网络策略              | **支持**                  |                                                              |



面对这么多选项，可以从以下几个角度来决策：

- **看规模和性能**：
    - 如果是**小规模或测试集群**，追求简单快速，**Flannel** 是首选。
    - 如果是**大规模生产集群**，对性能和网络策略有要求，**Calico** 是各方面都很均衡的成熟选择。
    - 如果对**性能、安全性和可观测性**有极致追求（例如金融、实时数据处理），**Cilium** 代表了未来的技术方向。
- **看功能需求**：
    - **需要网络策略 (NetworkPolicy)**：就避开 Flannel，在 Calico、Cilium、Antrea 等支持策略的插件中选择。
    - **需要加密通信**：Calico（支持WireGuard）、Cilium（支持IPsec/WireGuard）和 Weave Net 都支持。
    - **需要多网络接口**：则需要考虑 **Multus**



### 62、**Kubernetes 网络插件有哪些优势？**
...
### 71、请解释 Kubernetes 中的 Helm 及其用途。
...
### 73、**请简述 Kubernetes 中的服务网格(Service Mesh)概念。**
...
### 74、**如何在 Kubernetes 中实现 CI/CD 流水线？**
